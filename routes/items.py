...
def index():
    ...
    # Get wishlist item IDs for the logged-in user so card icons show state
    wishlist_ids = set()
    if session.get('user_id'):
        wishlist_rows = Wishlist.list_for_user(session['user_id'])
        wishlist_ids = {row['id'] for row in wishlist_rows}
...
def buy_item(item_id):
    ...
    flash('Purchase successful! You can now contact the seller to arrange delivery.', 'success')
    
    # Redirect to chat with seller for transaction coordination
    return redirect(url_for('chat.conversation', recipient_id=item['owner_id']))
...
def add_wishlist(item_id):
    ...
    return redirect(request.referrer or url_for('main.item_detail', item_id=item_id))
